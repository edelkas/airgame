...
set L;			#Conjunto de casillas en las que colocar la base aérea
...
param G {j in L} binary;	#Existe supremacía aérea del propio jugador o no en esa casilla
...
#Restricciones:
#1) No se puede colocar la misma base aérea en más de una casilla
R1 sum X[j] =1

#2) Se debe colocar la ciudad en una casilla con supremacía aérea del propio jugador
R2 A[j] =1
